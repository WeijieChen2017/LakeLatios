...
class MONAI_ViTAutoEnc(nn.Module):
    def __init__(
        self,
        img_size: int = 1024,
        patch_size: int = 16,
        ...
        self.convTrans_1 = conv_trans(nc8, nc4, kernel_size=3, stride=2, bias=False)
        self.convTrans_2 = conv_trans(nc4, nc2, kernel_size=3, stride=2, bias=False)
        self.convTrans_3 = conv_trans(nc2, nc, kernel_size=3, stride=2, bias=False)
        self.convTrans_4 = conv_trans(nc, nc, kernel_size=3, stride=2, bias=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.verbose:
            print("after convTrans_1 x.shape", x.shape, "means", x.mean(), "std", x.std())
        x = self.convTrans_2(x)
        if self.verbose:
            print("after convTrans_2 x.shape", x.shape, "means", x.mean(), "std", x.std())
        x = self.convTrans_3(x)
        if self.verbose:
            print("after convTrans_3 x.shape", x.shape, "means", x.mean(), "std", x.std())
