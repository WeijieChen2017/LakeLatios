...
class MONAI_UNETR(nn.Module):
    def __init__(
        ...
        self.z6_block = nn.Sequential(
            conv_trans(ebd, nc8, kernel_size=4, stride=2, padding=1, bias=False),
            conv_trans(nc8, nc4, kernel_size=4, stride=2, padding=1, bias=False),
        )
        self.z3_block = nn.Sequential(
            conv_trans(ebd, nc8, kernel_size=4, stride=2, padding=1, bias=False),
            conv_trans(nc8, nc4, kernel_size=4, stride=2, padding=1, bias=False),
            conv_trans(nc4, nc2, kernel_size=4, stride=2, padding=1, bias=False),
        )

        self.decoder_12 = nn.Sequential(
            conv(nc8+nc8, nc8, kernel_size=3, padding=1, bias=False),
            conv_trans(nc8, nc8, kernel_size=4, stride=2, padding=1, bias=False),
        )

        self.decoder_9 = nn.Sequential(
            conv(nc8+nc8, nc8, kernel_size=3, padding=1, bias=False),
            conv(nc8, nc4, kernel_size=3, padding=1, bias=False),
            conv_trans(nc4, nc4, kernel_size=4, stride=2, padding=1, bias=False),
        )

        self.decoder_6 = nn.Sequential(
            conv(nc4+nc4, nc4, kernel_size=3, padding=1, bias=False),
            conv(nc4, nc2, kernel_size=3, padding=1, bias=False),
            conv_trans(nc2, nc2, kernel_size=4, stride=2, padding=1, bias=False),
        )

        self.decoder_3 = nn.Sequential(
            conv(nc2+nc2, nc2, kernel_size=3, padding=1, bias=False),
            conv(nc2, nc, kernel_size=3, padding=1, bias=False),
            conv_trans(nc, nc, kernel_size=4, stride=2, padding=1, bias=False),
        )
        ...
        self.decoder_out = nn.Sequential(
            conv(nc+nc, nc, kernel_size=3, padding=1, bias=False),
            conv(nc, out_chans, kernel_size=3, padding=1, bias=False),
        )
